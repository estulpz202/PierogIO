...
describe('Order Calculations', () => {
  ...
  describe('total', () => {
    ...
    it('should calculate order total with skipping delivery', () => {
      const order = {
        items: [
          {
            sku: 'P6-POTATO', // could be any valid SKU (see README.md for examples)
            ...
            unitPriceCents: 500, // price per unit in cents
            addOns: [], // could include 'sour-cream', 'fried-onion', 'bacon-bits'
          }
        ]
      };
      ...
      expect(orderTotal).toBeGreaterThan(0);
      expect(Number.isInteger(orderTotal)).toBe(true);
      expect(orderTotal).toBe((10*500) + 0 + 0); // subtotal + deliveryFee + tax
    });
  });
